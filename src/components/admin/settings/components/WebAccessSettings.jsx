...
import { Globe, RefreshCw, Clock, User, UserX, UserCheck, Filter } from 'lucide-react';
        ...
        <div className="w-full bg-white rounded-[24px] border border-[#f2f4f6] p-6 shadow-sm flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-100 pb-5">
                <div>
                    <h3 className="text-lg font-bold text-[#191f28] flex items-center gap-2 tracking-tight">
                        <Globe className="text-[#3182f6]" size={22} />
                        최근 웹 접속 현황 {excludeStaff ? '(학생 전용 상위 20명)' : '(상위 20명)'}
                    </h3>
                    <p className="text-xs md:text-sm text-[#8b95a1] mt-1 font-medium leading-relaxed">
                        {excludeStaff
                            ? '스탭/관리자를 제외하고 학생 이용자의 최근 웹페이지 접속 시각(한국 시간 KST)을 보여줍니다.'
                            : '서비스 웹페이지에 직접 접속한 이용자를 가장 최근 접속 시각(한국 시간 KST) 기준으로 보여줍니다.'}
                    </p>
                </div>
                <div className="flex items-center gap-2 self-end sm:self-auto">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl font-bold transition-all text-xs border shrink-0 ${
                            excludeStaff
                                ? 'bg-blue-600 text-white border-blue-600 shadow-sm hover:bg-blue-700'
                                : 'bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100'
                        }`}
                    >
                        {excludeStaff ? <UserX size={15} /> : <UserCheck size={15} />}
                        <span>스탭 제외</span>
                    </button>
                    {/* Refresh Button */}
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-200 text-gray-700 hover:bg-gray-100 rounded-xl font-bold transition-all shadow-xs text-xs shrink-0 active:scale-95 disabled:opacity-50"
                    >
                        <RefreshCw size={15} className={isRefreshing ? 'animate-spin text-[#3182f6]' : ''} />
                        {isRefreshing ? '갱신 중...' : '접속 현황 새로고침'}
                    </button>
                </div>
            </div>
            ...
            {recentWebUsers.length === 0 ? (
                <div className="py-16 text-center text-gray-400 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
                    <Globe size={40} className="mx-auto mb-3 text-gray-300 stroke-[1.5]" />
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/80 text-gray-500 text-xs font-bold border-b border-gray-100">
                                ...
                                <th className="py-3.5 px-4">소속 / 구분</th>
                                <th className="py-3.5 px-4 text-right sm:text-left">최근 웹 접속 일시 (KST)</th>
                                    ...
                                    <tr key={user.id || index} className="hover:bg-blue-50/30 transition-colors">
                                        ...
                                        <td className="py-3.5 px-4 font-bold text-[#191f28]">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="truncate max-w-[120px] sm:max-w-[180px]">{user.name || '미등록'}</span>
                                                {user.is_leader && (
                                        ...
                                        <td className="py-3.5 px-4 text-gray-600">
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-1">
                                                ...
                                                {user.school && (
                                                    <span className="text-gray-400 text-xs hidden sm:inline">• {user.school}</span>
                                                )}
                                            </div>
                                        </td>

                                        {/* KST Access Time */}
                                        <td className="py-3.5 px-4 font-mono font-medium text-gray-700 text-right sm:text-left text-xs sm:text-sm">
